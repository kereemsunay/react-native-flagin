...
export default class Game extends Component {
  ...
  render() {
    ...
    const sound_ta = new Sound(
      require("../assets/sound/trueAnswer.wav"),
      null,
      (error) => {
        if (error) {
    ...
    const sound_fa = new Sound(
      require("../assets/sound/falseAnswer.wav"),
      null,
      (error) => {
        if (error) {
          console.log("Ses yüklenemedi");
    ...
    SecimFunc = (index) => {
      degisken = randomFunc();
      global.correct = data[index].name;
      if (degisken == "1") {
    ...
    chooseCountry = (index) => {
      var newCountry = data[Math.floor(Math.random() * data.length)].name;
      while (newCountry == data[index].name) {
        newCountry = data[Math.floor(Math.random() * data.length)].name; //yanlış eşleşene kadar değiştir.
      }
      ...
      <SafeAreaView style={styles.container}>
        ...
        >
          <View style={{ alignItems: "center", height: "30%" }}>
            <Text style={[styles.text, styles.heading]}>Score: {count}</Text>
            <Text
              style={{
                fontSize: 16,
                textAlign: "center",
